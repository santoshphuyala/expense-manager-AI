...
// ==========================================
// IMPORT/EXPORT FUNCTIONS - ENHANCED WITH OPTIONS
// Place these INSIDE your ExpenseTrackerApp class
// ==========================================

// Generate unique fingerprint for transaction
generateTransactionFingerprint(transaction) {
    try {
        const key = `${transaction.date}_${transaction.type}_${transaction.category}_${transaction.description}_${transaction.amount}_${transaction.account}`;
        const fingerprint = key.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9_]/g, '');
        console.log('🔑 Generated fingerprint:', fingerprint);
        return fingerprint;
    } catch (error) {
        console.error('❌ Error generating fingerprint:', error, transaction);
        return null;
    }
}

async isDuplicateTransaction(transaction, existingTransactions = null) {
    try {
        const transactions = existingTransactions || await this.db.getAll('transactions');
        const newFingerprint = this.generateTransactionFingerprint(transaction);
        if (!newFingerprint) return false;

        const duplicate = transactions.find(existing => {
            const existingFingerprint = this.generateTransactionFingerprint(existing);
            return existingFingerprint === newFingerprint;
        });

        if (duplicate) {
            console.log('🔄 Duplicate found:', transaction.description);
        }

        return duplicate || false;
    } catch (error) {
        console.error('❌ Error checking duplicate:', error, transaction);
        return false;
    }
}

validateTransaction(transaction, lineNumber = null) {
    const errors = [];
    const warnings = [];

    try {
        console.log(`🔍 Validating transaction${lineNumber ? ` (Line ${lineNumber})` : ''}:`, transaction);

        // Required fields
        const required = ['date', 'type', 'category', 'description', 'amount', 'account'];
        ...
        if (transaction.date) {
            ...
            if (isNaN(date.getTime())) {
                ...
                warnings.push(`Future date detected: ${transaction.date}`);
            }
        }
        ...
        if (transaction.type && !validTypes.includes(transaction.type.toLowerCase())) {
            errors.push(`Invalid type: ${transaction.type}. Must be one of: ${validTypes.join(', ')}`);
        }
        ...
        if (transaction.amount !== undefined && transaction.amount !== null) {
            ...
            if (isNaN(amount)) {
                ...
                warnings.push(`Unusually large amount: ${amount}`);
            }
        }

        // Description validation
        if (transaction.description && transaction.description.length > 500) {
            warnings.push('Description exceeds 500 characters');
        }
        ...
        console.error('❌ Validation error:', error, transaction);
        return {
            valid: false,
...
async exportData(format) {
    ...
    try {
        ...
        if (format === 'json') {
            ...
            this.downloadFile(data, `transactions-${timestamp}.json`, 'application/json');
            console.log(`✅ JSON exported`);
        } else if (format === 'csv') {
            const csv = this.convertToCSV(transactions);
            this.downloadFile(csv, `transactions-${timestamp}.csv`, 'text/csv');
            console.log(`✅ CSV exported (${csv.length} bytes)`);
            ...
            const wsData = transactions.map((t, index) => ({
                ID: index + 1,
                Date: t.date || '',
                Type: t.type || '',
                Category: t.category || '',
        ...
        this.showToast(`Successfully exported ${transactions.length} transactions as ${format.toUpperCase()}!`, 'success');
...
async exportFullBackup() {
    ...
    try {
        ...
        console.log('📦 Backup data retrieved:', backup);
        ...
        if (backup.data) {
            for (const [store, records] of Object.entries(backup.data)) {
                backup.metadata.recordCount[store] = Array.isArray(records) ? records.length : 0;
                console.log(`📊 ${store}: ${backup.metadata.recordCount[store]} records`);
            }
        }
        ...
        this.showToast('Full backup created successfully!', 'success');
...
convertToCSV(data) {
    try {
        ...
        const headers = ['ID', 'Date', 'Type', 'Category', 'Description', 'Amount', 'Account', 'Currency', 'Notes', 'CreatedAt'];
        ...
        const rows = data.map((t, index) => [
            ...
            t.amount || 0,
            escapeCSV(t.account),
            escapeCSV(t.currency || 'NPR'),
            escapeCSV(t.notes),
            escapeCSV(t.createdAt)
        ].join(','));

        const csv = [headers.join(','), ...rows].join('\n');
        console.log(`✅ CSV conversion complete: ${rows.length} rows`);
...
async importData() {
    ...
    if (!fileInput) {
        console.error('❌ File input not found: #importFile');
        this.showToast('Import button not found', 'error');
        return;
    }
    ...
    if (file.size > 50 * 1024 * 1024) {
        ...
        this.showToast('File too large! Maximum size: 50MB', 'error');
        return;
    }
    ...
    // Parse file first to analyze it
    try {
        const parsedData = await this.parseImportFile(file);
        await this.showImportOptionsDialog(parsedData);
    } catch (error) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (filename.endsWith('.json')) {
                    ...
                    transactions = await this.parseJSONContent(content);
                } else if (filename.endsWith('.csv')) {
                    console.log('🔧 Processing CSV file...');
                    transactions = await this.parseCSVContent(content);
                } else if (filename.endsWith('.xlsx') || filename.endsWith('.xls')) {
                    console.log('🔧 Processing Excel file...');
                    transactions = await this.parseExcelContent(content);
                } else {
                    throw new Error(`Unsupported file format: ${file.name}`);
                }

                // Analyze transactions
                const analysis = await this.analyzeImportData(transactions);
...
async analyzeImportData(transactions) {
    console.log('🔍 Analyzing import data...');

    const existingTransactions = await this.db.getAll('transactions');
    const analysis = {
    ...
    for (let i = 0; i < transactions.length; i++) {
        ...
        // Validate
        const validation = this.validateTransaction(txn, i + 1);
        if (validation.valid) {
            analysis.valid++;
            
            // Check for duplicates
            const isDupe = await this.isDuplicateTransaction(txn, existingTransactions);
            if (isDupe) {
                analysis.duplicates++;
                txn._isDuplicate = true;
                txn._existingId = isDupe.id;
            } else {
                analysis.new++;
                txn._isDuplicate = false;
            }
        } else {
            analysis.invalid++;
...
async showImportOptionsDialog(parsedData) {
    const { transactions, analysis } = parsedData;
    ...
    if (!modal) {
        console.error('❌ Import options modal not found');
        // Fallback to simple import
        await this.executeImport(transactions, 'merge');
        return;
    }

    // Update modal content
    document.getElementById('importTotalCount').textContent = analysis.total;
    document.getElementById('importValidCount').textContent = analysis.valid;
    document.getElementById('importInvalidCount').textContent = analysis.invalid;
    document.getElementById('importDuplicateCount').textContent = analysis.duplicates;
    document.getElementById('importNewCount').textContent = analysis.new;

    // Show/hide warning sections
    const errorsSection = document.getElementById('importErrorsSection');
    const warningsSection = document.getElementById('importWarningsSection');
    
    if (analysis.errors.length > 0) {
        errorsSection.style.display = 'block';
        const errorList = document.getElementById('importErrorsList');
        errorList.innerHTML = analysis.errors.slice(0, 5).map(err => 
            `<li><strong>Line ${err.line}:</strong> ${err.errors.join(', ')}</li>`
        ).join('');
        if (analysis.errors.length > 5) {
            errorList.innerHTML += `<li><em>...and ${analysis.errors.length - 5} more errors</em></li>`;
        }
    } else {
    ...
    if (analysis.warnings.length > 0) {
        ...
        const warningList = document.getElementById('importWarningsList');
        warningList.innerHTML = analysis.warnings.slice(0, 5).map(warn => 
            `<li><strong>Line ${warn.line}:</strong> ${warn.warnings.join(', ')}</li>`
        ).join('');
        if (analysis.warnings.length > 5) {
            warningList.innerHTML += `<li><em>...and ${analysis.warnings.length - 5} more warnings</em></li>`;
        }
    } else {
        warningsSection.style.display = 'none';
    }

    // Enable/disable options based on data
    const existingCount = await this.db.count('transactions');
    document.getElementById('importExistingCount').textContent = existingCount;

    const overwriteBtn = document.getElementById('importOverwriteBtn');
    const mergeBtn = document.getElementById('importMergeBtn');

    // Disable overwrite if no existing data
    if (existingCount === 0) {
        overwriteBtn.disabled = true;
        overwriteBtn.title = 'No existing data to overwrite';
    } else {
        overwriteBtn.disabled = false;
        overwriteBtn.title = '';
    }

    // Disable merge if no new records
    if (analysis.new === 0) {
        mergeBtn.disabled = true;
        mergeBtn.title = 'No new records to add';
    } else {
...
async executeImport(transactions, mode) {
    console.log(`🚀 Executing import with mode: ${mode}`);
    const startTime = performance.now();

    this.closeImportOptionsDialog();
    this.showToast(`Importing data (${mode} mode)... Please wait`, 'info');

    try {
        let result = { imported: 0, duplicates: 0, errors: 0, errorDetails: [], deleted: 0 };
        ...
        if (mode === 'overwrite') {
            ...
            console.log(`✅ Deleted ${existingCount} existing transactions`);

            // Import all valid transactions (including duplicates)
            result = await this.importTransactions(transactions, false); // Don't check duplicates
            result.deleted = existingCount;

        } else if (mode === 'merge') {
            // Import only new transactions
            result = await this.importTransactions(transactions, true); // Check duplicates
        ...
        if (mode === 'overwrite') {
            message = `✅ Replaced ${result.deleted} records with ${result.imported} new records`;
        } else if (result.imported > 0) {
            message = `✅ Added ${result.imported} new records`;
            if (result.duplicates > 0) {
                message += `, skipped ${result.duplicates} duplicates`;
            }
        } else {
            message = 'No new records imported';
            type = 'info';
        }
        ...
        if (result.imported > 0 || result.deleted > 0) {
            console.log('🔄 Reloading dashboard and transactions...');
            await this.loadDashboard();
            await this.loadTransactions();
        }
...
async parseJSONContent(content) {
    ...
    if (data.data && typeof data.data === 'object') {
        console.log('📦 Detected full backup format');
        if (data.data.transactions && Array.isArray(data.data.transactions)) {
            return data.data.transactions;
        }
    } else if (Array.isArray(data)) {
        console.log('📋 Detected transaction array format');
        return data;
    }
...
async parseCSVContent(content) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (values.length < 6) {
            console.warn(`⚠️ Line ${i + 1}: Insufficient columns`);
            continue;
        }

        // Skip ID column if present
        let offset = 0;
        if (!isNaN(values[0]) && values[0].trim().match(/^\d+$/)) {
            offset = 1;
        }

        const transaction = {
            date: values[offset]?.trim(),
            type: values[offset + 1]?.trim(),
            category: values[offset + 2]?.trim(),
            description: values[offset + 3]?.replace(/"/g, '').trim(),
            amount: parseFloat(values[offset + 4]),
            account: values[offset + 5]?.trim(),
            currency: values[offset + 6]?.trim() || 'NPR',
            notes: values[offset + 7] ? values[offset + 7].replace(/"/g, '').trim() : '',
            createdAt: values[offset + 8]?.trim() || new Date().toISOString()
        };
...
async parseExcelContent(content) {
    if (typeof XLSX === 'undefined') {
        throw new Error('XLSX library not loaded. Please include SheetJS library.');
    }
    ...
    const transactions = jsonData.map(row => ({
        ...
        currency: row.Currency || row.currency || 'NPR',
...
async importTransactions(transactions, checkDuplicates = true) {
    ...
    const existingTransactions = checkDuplicates ? await this.db.getAll('transactions') : [];
    ...
    for (let i = 0; i < transactions.length; i++) {
        ...
        try {
            ...
            const validation = this.validateTransaction(txn, i + 1);
            if (!validation.valid) {
                console.warn(`⚠️ Transaction ${i + 1} failed validation:`, validation.errors);
                errors++;
                errorDetails.push({ line: i + 1, error: validation.errors.join('; ') });
                continue;
            }
            ...
            const newTxn = { ...txn };
            delete newTxn.id;
            delete newTxn._isDuplicate;
            delete newTxn._existingId;
            newTxn.createdAt = newTxn.createdAt || new Date().toISOString();

            await this.db.add('transactions', newTxn);
            console.log(`✅ Transaction ${i + 1} imported`);
            imported++;

        } catch (error) {
            console.error(`❌ Error importing transaction ${i + 1}:`, error);
            errors++;
            errorDetails.push({ line: i + 1, error: error.message });
        }
    }

    console.log('✅ Import complete:', { imported, duplicates, errors });
...
async clearAllData() {
    console.log('🗑️ Clear all data initiated');
    ...
    try {
        const stores = ['transactions', 'shopping_lists', 'subscriptions', 'wishlist', 'documents', 'insights', 'notifications'];
        console.log('🗑️ Clearing stores:', stores);
